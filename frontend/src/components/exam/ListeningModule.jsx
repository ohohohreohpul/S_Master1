  ...
  const renderFallbackQuestion = (q) => {
    ...
    return (
      <div key={qNum} className={`question-block ${isFlagged ? "border-[var(--ps-orange)]" : ""}`} data-testid={`question-${qNum}`}>
        ...
        {(q.question_type === "richtig_falsch" ||
          q.correct_answer === "Richtig" || q.correct_answer === "Falsch") ? (
          <div className="flex gap-3">
            {["Richtig", "Falsch"].map(opt => (
              <label key={opt} className={`flex items-center gap-2 px-4 py-2 rounded-xl border cursor-pointer transition-all text-sm font-medium ${
                answers[qNum] === opt
                  ? "border-[var(--ps-blue)] bg-[var(--ps-blue)]/5 text-[var(--ps-blue)]"
                  : "border-[var(--ps-divider)] hover:border-[var(--ps-blue)]/40"
              }`} data-testid={`option-${qNum}-${opt}`}>
                <input type="radio" name={`q_${qNum}`} value={opt} checked={answers[qNum] === opt}
                  onChange={(e) => updateAnswer(qNum, e.target.value)} className="hidden" />
                {opt}
              </label>
            ))}
          </div>
        ...
        <div className="exam-left">
          ...
          <div className="mb-4 pb-4 border-b border-[var(--ps-divider)]">
            ...
            <p className="text-sm text-[var(--ps-body-gray)] mt-1">
              {isTelc ? "Hören — Richtig oder Falsch?" : "Listen and answer questions"}{" "}
              {section?.questions?.[0]?.question_num} - {section?.questions?.[section.questions.length - 1]?.question_num}
            </p>
          </div>
